...
# --- CONFIGURAÇÕES ---
CAMINHO_DIRETORIO = "notas_de_corretagem"
ARQUIVO_SAIDA_TXT = "notas_extraidas.txt"
...
def processar_arquivos_pdf():
    ...
    if not os.path.isdir(CAMINHO_DIRETORIO):
        logging.error(f"O diretório '{CAMINHO_DIRETORIO}' não foi encontrado.")
        return

    notas_agrupadas = {}
    arquivos_pdf = sorted([f for f in os.listdir(CAMINHO_DIRETORIO) if f.lower().endswith('.pdf')])
    logging.info(f"Encontrados {len(arquivos_pdf)} arquivos PDF para processar.")
    ...
    for numero_nota, dados_nota in notas_agrupadas.items():
        logging.info(f"=== Processando e Calculando Nota: {numero_nota} ===")
    ...
    if resultados_finais_formatados:
        with open(ARQUIVO_SAIDA_TXT, 'w', encoding='utf-8') as f:
            f.write('\n'.join(resultados_finais_formatados))
        logging.info(f"\nProcessamento concluído! Resultados salvos em '{ARQUIVO_SAIDA_TXT}'")
